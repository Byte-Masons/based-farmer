...
contract ReaperAutoCompoundBasedFarmer is ReaperBaseStrategy {
     ...
     * {receiptToken} - Receipt for adding liquidity to Curve
    ...
    function _addLiquidity() internal {
        ...
        uint256 receiptBalance = IERC20Upgradeable(receiptToken).balanceOf(address(this));
        ICurveGauge(want).deposit(receiptBalance);
    }
    ...
    function _giveAllowances() internal {
        ...
        // receiptToken -> want
        uint256 receiptTokenAllowance = type(uint256).max - IERC20Upgradeable(receiptToken).allowance(address(this), want);
        IERC20Upgradeable(receiptToken).safeIncreaseAllowance(want, receiptTokenAllowance);
    ...
    function _removeAllowances() internal {
        ...
        IERC20Upgradeable(receiptToken).safeDecreaseAllowance(
            want,
            IERC20Upgradeable(receiptToken).allowance(address(this), want)
        );

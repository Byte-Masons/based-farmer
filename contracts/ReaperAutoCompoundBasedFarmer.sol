...
contract ReaperAutoCompoundBasedFarmer is ReaperBaseStrategy {
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20Upgradeable(want).safeTransfer(vault, wantBal - withdrawFee);
    }
    ...
    function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
        uint256 pendingReward = IMasterChef(MASTER_CHEF).pendingShare(
            poolId,
            address(this)
        );

        uint256 freeRewards = IERC20Upgradeable(BSHARE)
        ...
        uint256[] memory amountOutMins = IUniswapV2Router02(SPOOKY_ROUTER)
            .getAmountsOut(totalRewards, rewardToWftmPath);
    ...
    function _addLiquidity() internal {
        ...
        if (bshareBalance != 0) {
            _swapTokens(BSHARE, WFTM, bshareBalance / 2);
            uint256 lp0Bal = IERC20Upgradeable(lpToken0).balanceOf(address(this));
            uint256 lp1Bal = IERC20Upgradeable(lpToken1).balanceOf(address(this));
            IUniswapV2Router02(SPOOKY_ROUTER).addLiquidity(lpToken0, lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
        }
    }
